...
# @celery_app.task(bind=True)
# def crawl_website_task(self, client_id: str, allowed_domain: str, start_url: str, max_pages: int = 50):
...
#     # Candidate script paths (try common places in repo)
#     candidate_paths = [
#         # os.path.abspath(os.path.join(BASE_DIR, "../Chatbot/crawler/playwright_scraper.py")),
#         # os.path.abspath(os.path.join(CRAWLER_DIR, "playwright_scraper.py")),
#         # os.path.abspath(os.path.join(CRAWLER_DIR, "crawler", "spiders", "playwright_scraper.py")),
#         os.path.abspath(os.path.join(CRAWLER_DIR, "crawler", "spiders", "website_scrap.py")),
#         # os.path.abspath(os.path.join(CRAWLER_DIR, "playwright_scraper.py")),  # final fallback
#     ]
...
@celery_app.task(bind=True)
def crawl_and_embed_pipeline(self, client_id: str, allowed_domain: str, start_url: str, max_pages: int = 50):
    """High-level pipeline for crawling website + embeddings."""
    logger.info(f"Starting combined crawl+embed for client {client_id}")
